...
# Usage:
#
#   julia fvgor.jl g2 N D lmax [--pbc] [--excited] [--outdir data] [--tol 1e-8] [--maxiter 100] [--verbosity 3] [--penalty 100.0] [--device gpu|cpu|auto] [--gpu-id 0] [--allow-scalar false] [--unified-memory false]
#
# Example:
#
#   julia fvgor.jl 1.0 40 400 4 --device gpu --gpu-id 0 --unified-memory true
#
# Hamiltonian:
#
#   H = sum_i (g²/2) L_i² - (1/g²) sum_i n_i · n_{i+1}
#
# Real-basis variant:
...
#
# By default open boundary conditions are used. Use --pbc to include the wraparound bond n_N · n_1.
#
...
if length(ARGS) < 4
    println("Usage: julia fvgor.jl g2 N D lmax [--pbc] [--excited] [--outdir data] [--tol 1e-8] [--maxiter 100] [--verbosity 3] [--penalty 100.0] [--device gpu|cpu|auto] [--gpu-id 0] [--allow-scalar false] [--unified-memory false]")
    println("Example: julia fvgor.jl 1.0 40 400 4 --device gpu --gpu-id 0")
    exit(1)
end
...
# Open boundary conditions are now the default.
# Use --pbc to include the wraparound N--1 bond.
# --open is still accepted as a no-op compatibility flag.
const periodic_boundary = has_flag(ARGS, "--pbc")
const open_boundary = !periodic_boundary
...
if has_flag(ARGS, "--open") && has_flag(ARGS, "--pbc")
    error("Use either --open or --pbc, not both. Open boundary conditions are the default; --open is optional.")
end
...
# ============================================================
# Finite Hamiltonian: OBC by default, PBC with --pbc
# ============================================================
...
function connected_pbc_correlator(ψ::MPS, sites, ops; N::Int, use_gpu::Bool = USE_CUDA)
    # PBC is kept as a correctness fallback. For production timing, use the
    # default OBC path, which calls connected_obc_correlator_sweep above.
    use_gpu = use_gpu && USE_CUDA
