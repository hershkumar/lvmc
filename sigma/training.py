...
def _ensure_configs_sharded(configs, sharding):
    if getattr(configs, "sharding", None) == sharding:
        return configs
    return jax.device_put(configs, sharding)
...
def train_adapt_shard(
    ...
    if not MC_options["chain"]:
        for step_num in pbar:
            ...
            samples = _ensure_configs_sharded(samples, configs_sharding)
